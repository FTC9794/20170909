...
@Autonomous(name = "Omni Directional Auto Demo", group = "Beta Structure")
public class OmniDirectionalAutonomousDemo extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()&&drive.move(.5, MIN_POWER, 12*COUNTS_PER_INCH-drive.averageEncoders(), .003, 0, .025, .01, 0, 24*COUNTS_PER_INCH-drive.averageEncoders()<ENCODER_OFFSET&&24*COUNTS_PER_INCH-drive.averageEncoders()>-ENCODER_OFFSET, 500)){
            telemetry.addData("power", 24*COUNTS_PER_INCH-drive.averageEncoders()*.003);
        }
        drive.resetEncoders();

        while(opModeIsActive()&&drive.move(.5, MIN_POWER, 0, 0, 0, .025, .01, 45, true, 500));
